...
import { Rating } from './Home/components/Ratting';
        ...
        <section>
          ...
          <div className="flex flex-wrap justify-around">
            ...
            <div className="max-w-xl my-3">
              ...
              <p className="my-3">
               { !incart && <button onClick={()=>addToCart(product)} disabled={product.in_stock?"":"disabled"} className={ `inline-flex items-center py-2 px-5 text-lg font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800M ${product.in_stock ? "" : "cursor-not-allowed"} `} >Add To Cart <i className="ml-1 bi bi-plus-lg"></i></button>}
                {incart && <button onClick={()=>removeFromCart(product)} disabled={product.in_stock?"":"disabled"} className="inline-flex items-center py-2 px-5 text-lg font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800" >Remove Item <i className="ml-1 bi bi-trash3"></i></button>}
              </p>
